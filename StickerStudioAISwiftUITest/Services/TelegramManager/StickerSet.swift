...
class StickerSet {
        ...
        switch data {
            case let .image(imageData):
                ...
                if let image = UIImage(data: imageData) {
                    ...
                    if Int(image.size.width) == TelegramStickersLimits.stickerDimensionsSide {
                        if Int(image.size.height) > 0 && Int(image.size.height) <= TelegramStickersLimits.stickerDimensionsSide {
                            dimensionsValid = true
                        }
                    } else if Int(image.size.height) == TelegramStickersLimits.stickerDimensionsSide {
                        if Int(image.size.width) > 0 && Int(image.size.width) <= TelegramStickersLimits.stickerDimensionsSide {
                            dimensionsValid = true
                        }
                    }
                    if !dimensionsValid {
            ...
            case let .animation(animationData):
                ...
                if animationData.count > TelegramStickersLimits.animatedStickerMaxSize {
            ...
            case let .video(videoData):
                ...
                if videoData.count > TelegramStickersLimits.videoStickerMaxSize {
                    throw StickersError.fileTooBig
                }
        }
        return true
    ...
    public func addSticker(data: Sticker.StickerData, emojis: [String]) throws {
        if self.stickers.count == TelegramStickersLimits.stickerSetStickerMaxCount {
